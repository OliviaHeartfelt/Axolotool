...
namespace NDWire {
    ...
    public:
        ...
        bool importWireCore(const QJsonObject& doc, QSqlQuery& query, bool isResourceOptional = true) {
            QJsonValue jsonValue = doc.value("wire_core");
            ...
            for (const QJsonValue& val : jsonArray) {
                ...
                auto optNewRecord = NDWireDetails::Config::CreateWireCoreRecord::Parse(val.toObject());
                if (!optNewRecord) return false;

                if (!NDWireDetails::Create::createWireCore(query, *optNewRecord)) return false;
            }
            return true;
        }
        bool importWireTemporary(const QJsonObject& doc, QSqlQuery& query, bool isResourceOptional = true) {
            QJsonValue jsonValue = doc.value("wire_temporary");
            if (jsonValue.isUndefined()) return isResourceOptional;
            if (!jsonValue.isArray()) return false;
            QJsonArray jsonArray = jsonValue.toArray();
            ...
            for (const QJsonValue& val : jsonArray) {
                ...
                auto optNewRecord = NDWireDetails::Config::CreateWireTemporaryRecord::Parse(val.toObject());
                if (!optNewRecord) return false;

                if (!NDWireDetails::Create::createWireTemporary(query, *optNewRecord)) return false;
            }
            return true;
        }
        bool importWire(const QJsonObject& doc, QSqlQuery& query, bool isResourceOptional = true) {
            QJsonValue jsonValue = doc.value("wire");
            if (jsonValue.isUndefined()) return isResourceOptional;
            if (!jsonValue.isArray()) return false;
            QJsonArray jsonArray = jsonValue.toArray();
            ...
            for (const QJsonValue& val : jsonArray) {
                ...
                auto optNewRecord = NDWireDetails::Config::CreateWireRecord::Parse(val.toObject());
                if (!optNewRecord) return false;

                if (!NDWireDetails::Create::createWire(query, *optNewRecord)) return false;
            }
            return true;
        }
