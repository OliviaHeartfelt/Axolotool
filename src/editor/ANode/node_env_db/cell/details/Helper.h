...
namespace NDCellDetails::Helper {
    ...
    inline bool isCellAvailable(QSqlQuery& query, const muuid::uuid& nodeId, const NDCellDetails::Config::RecordInfo& cellInfo, const std::optional<muuid::uuid>& ignoreCellId = std::nullopt) {
        QString checkSQL = R"(
    ...
    inline bool removeCollidingCells(QSqlQuery& query, const muuid::uuid& nodeId, const NDCellDetails::Config::RecordInfo& cell) {
        query.prepare(R"(
